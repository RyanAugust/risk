...
class BasicRiskBot(AbstractRiskPlayer):
    ...
    def move_after_attack(self, game_master, origin_name, target_name):
        ...
        if armies_to_move == 0:
            armies_to_move = 1
        origin_armies, destination_armies = game_master.player_move_armies(
            self, origin_name, target_name, int(armies_to_move))
        risk.logger.debug("%s now has: %s armies" %
                          (origin_name, origin_armies))
        risk.logger.debug("%s now has: %s armies" %
                          (target_name, destination_armies))
        ...
        for name, territory in territories.items():
            if territory.armies > 2:
                ...
                if len(targets) > 0:
                    ...
                    success = game_master.player_attack(
                        self, name, targets[choice])
                    if success:
                        self.move_after_attack(
                            game_master, name, targets[choice])
                else:
        ...
        while self.reserves > 0 and continent_owned == 1:
            if continent == 'north_america':
                ...
                if territory.owner != self:
                    game_master.player_add_army(
                        self, 'central_america', half_of_reserves)
            ...
            for territory in game_master.board.continents[continent].values():
                if self.reserves > 0:
                    if territory.owner == self:
                        print(self.reserves)
                        game_master.player_add_army(
                            self, territory.name, self.reserves)
        if number_of_turns > 10:
            for territory in game_master.board.continents[continent].values():
                if self.reserves > 0:
                    if territory.owner != self:
...
class IanRiskBot(BasicRiskBot):
    ...
    def attack_others(self, game_master):
        ...
        for territory in territories:
            neighbours = sorted(
                territory.neighbours.values(), key=lambda x: x.armies)
            for neighbour in neighbours:
                if territory.armies > 100:
                    ...
                    game_master.player_attack(
                        self, territory.name, neighbour.name)
                    break
